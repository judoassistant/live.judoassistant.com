<template>
  ...
  <template v-if="tournamentsState == 'LOADED'">
    ...
    <Table v-if="upcomingTournaments.length > 0" :headers="headers" :rows="upcomingTournaments" v-slot="props" :pageSize=10>
      <TableColumn>
    ...
    <Table v-if="pastTournaments.length > 0" :headers="headers" :rows="pastTournaments" v-slot="props" :pageSize=10>
      <TableColumn>
        <router-link :to="{ name: 'tournament-home', params: { tournament: props.row.webName }}">{{ props.row.name }}</router-link>
      </TableColumn>
      <TableColumn>
